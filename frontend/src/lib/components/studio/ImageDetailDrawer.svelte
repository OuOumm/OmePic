<script lang="ts">
  ...
  async function loadIPDetail() {
    if (!preferences.adminToken || !targetIp) {
      ipDetail = null;
      loadedIpAddress = '';
      return;
    }
    ...
    try {
      const detail = await adminGetAbuseIPDetail(preferences.adminToken, currentIp);
  ...
  $effect(() => {
    if (image?.ip_address && image.ip_address !== loadedIpAddress) {
      imageLoaded = false;
      ipDetail = null;
      banTarget = null;
      loadIPDetail();
...
{#if image}
  <div class="fixed inset-0 z-[70] grid place-items-center p-2 sm:p-4" role="dialog" aria-modal="true" aria-label={t(preferences.language, 'admin.imageDetails')} tabindex="-1" onkeydown={handleKeydown} use:accessibleDialog={{ onClose }}>
    ...
    <dl class="grid grid-cols-2 gap-x-3 gap-y-1 text-xs sm:text-sm md:grid-cols-2">
      <div class="col-span-2 grid grid-cols-[3.75rem_minmax(0,1fr)_auto] items-center gap-1.5 border-b-2 border-dashed border-[hsl(var(--ink)/0.28)] py-1 md:col-span-2"><dt class="font-black">UID</dt><dd class="min-w-0 truncate font-bold" title={image.uid}>{image.uid}</dd><button class="studio-button px-1.5 py-1 sm:px-2 sm:py-1.5" type="button" onclick={() => copy(image.uid)} aria-label={t(preferences.language, 'image.copyUid')}><Copy class="size-3.5 sm:size-4" /></button></div>
      <div class="col-span-2 grid grid-cols-[3.75rem_minmax(0,1fr)_auto] items-center gap-1.5 border-b-2 border-dashed border-[hsl(var(--ink)/0.28)] py-1 md:col-span-2"><dt class="font-black">URL</dt><dd class="min-w-0 truncate" title={imageUrl}>{imageUrl}</dd><button class="studio-button px-1.5 py-1 sm:px-2 sm:py-1.5" type="button" onclick={() => copy(imageUrl)} aria-label={t(preferences.language, 'common.copyUrl')}><Copy class="size-3.5 sm:size-4" /></button></div>
      <div class="col-span-2 grid grid-cols-[3.75rem_minmax(0,1fr)_auto] items-center gap-1.5 border-b-2 border-dashed border-[hsl(var(--ink)/0.28)] py-1 md:col-span-2"><dt class="font-black">MD5</dt><dd class="min-w-0 truncate" title={image.md5_hash}>{image.md5_hash}</dd><button class="studio-button px-1.5 py-1 sm:px-2 sm:py-1.5" type="button" onclick={() => copy(image.md5_hash)} aria-label={t(preferences.language, 'image.copyMd5')}><Copy class="size-3.5 sm:size-4" /></button></div>
      <div class="col-span-2 grid grid-cols-[3.75rem_minmax(0,1fr)_auto] items-center gap-1.5 border-b-2 border-dashed border-[hsl(var(--ink)/0.28)] py-1 md:col-span-2"><dt class="font-black">Token</dt><dd class="min-w-0 truncate" title={image.token}>{image.token}</dd><button class="studio-button px-1.5 py-1 sm:px-2 sm:py-1.5" type="button" onclick={() => copy(image.token)} aria-label={t(preferences.language, 'image.copyToken')}><Copy class="size-3.5 sm:size-4" /></button></div>
      <div class="grid grid-cols-[3.75rem_minmax(0,1fr)] gap-1.5 border-b-2 border-dashed border-[hsl(var(--ink)/0.28)] py-1"><dt class="font-black">IP</dt><dd class="min-w-0 truncate" title={targetIpLabel}>{targetIpLabel}</dd></div>
