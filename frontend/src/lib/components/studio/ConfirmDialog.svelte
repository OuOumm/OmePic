...
{#if open}
  <div class="fixed inset-0 z-[110] grid min-h-dvh place-items-center overflow-x-hidden overflow-y-auto bg-[hsl(var(--ink)/0.48)] px-3 py-4 backdrop-blur-sm sm:p-6" role="presentation" onclick={(event) => event.target === event.currentTarget && !busy && onClose()} {@attach attachViewportPortal()}>
