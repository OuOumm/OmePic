<script lang="ts">
  ...
  const presets = $derived([
    ...
    { label: t(preferences.language, 'admin.securityDuration24Hours'), value: '24' },
    { label: t(preferences.language, 'admin.securityDuration7Days'), value: '168' },
    { label: t(preferences.language, 'admin.securityDuration30Days'), value: '720' },
    { label: t(preferences.language, 'admin.securityDurationPermanent'), value: 'permanent' },
    { label: t(preferences.language, 'admin.securityDurationCustom'), value: 'custom' },
  ]);
    ...
    <div class="w-full max-w-lg max-h-[calc(100dvh-2rem)] overflow-y-auto border-[3px] ink-line bg-[hsl(var(--paper))] p-4 shadow-[8px_8px_0_hsl(var(--ink))] sketch-enter sm:max-h-[calc(100dvh-3rem)] sm:p-5" role="dialog" tabindex="-1" aria-modal="true" aria-labelledby="ban-dialog-title" {@attach attachAccessibleDialog(() => ({ onClose }))}>
      <div class="mb-5 flex items-start justify-between gap-3 border-b-[3px] ink-line pb-3">
        <div>
          ...
          <h2 id="ban-dialog-title" class="mt-3 text-3xl font-black">{t(preferences.language, 'admin.securityBanIp')}</h2>
      ...
      <label class="grid gap-2 text-sm font-black">
        {t(preferences.language, 'admin.securityBanReason')}
        <textarea class="studio-input min-h-24" bind:value={reason}></textarea>
      </label>

      <div class="mt-5 grid gap-3">
        <p class="text-sm font-black">{t(preferences.language, 'admin.securityBanDuration')}</p>
        ...
        {#if selectedPreset === 'custom'}
          <label class="grid gap-2 text-sm font-black">
            {t(preferences.language, 'admin.securityCustomHours')}
      ...
      <div class="mt-6 flex flex-wrap justify-end gap-3 border-t-[3px] ink-line pt-4">
        <button class="studio-button" type="button" onclick={onClose}>{t(preferences.language, 'common.cancel')}</button>
        <button class="studio-button" data-tone="danger" type="button" disabled={busy || !reason.trim()} onclick={confirmBan}>{t(preferences.language, 'common.confirm')}</button>
      </div>
    </div>
  </div>
{/if}
