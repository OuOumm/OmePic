  ...
  <header class="sticky top-0 z-50 border-b-2 ink-line bg-[hsl(var(--paper)/0.86)] backdrop-blur-md">
    <div class="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 lg:px-6">
      <a href="/" class="group flex items-center gap-3 font-black tracking-tight">
        ...
        <span class="text-xl">OmePic</span>
      </a>

      <nav class="hidden items-center gap-1 lg:flex">
        {#each nav as item (item.href)}
          <a class="flex items-center gap-2 px-3 py-2 text-sm font-bold hover:marker-highlight" href={item.href}>
      ...
      <button class="studio-button lg:hidden" type="button" onclick={() => (menuOpen = !menuOpen)} aria-label="Menu">
        <Menu class="size-4" />
      </button>
    </div>

    {#if menuOpen}
      <nav class="grid border-t-2 ink-line bg-[hsl(var(--paper))] px-4 py-3 lg:hidden">
        {#each nav as item (item.href)}
          <a class="flex items-center gap-2 py-3 font-bold" href={item.href} onclick={() => (menuOpen = false)}>
            <item.icon class="size-4" />
            {item.label}
          </a>
        {/each}
