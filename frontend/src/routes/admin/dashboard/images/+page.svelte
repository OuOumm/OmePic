<script lang="ts">
  ...
  import { Archive, Ban, RotateCcw, Search, Trash2 } from 'lucide-svelte';
  ...
  import { adminCreateIPBan, adminDeleteImages, adminGetImages, adminGetTrashImages, adminRestoreImage } from '@/api';
  import { t } from '@/i18n';
  import { preferences } from '@/stores/preferences.svelte';
  import { isAbortError, formatBytes, formatDate, getImagePath } from '@/utils';
  import { runAsyncAction, toastApiError } from '@/ui-errors';
  import type { AdminImage, AdminTrashImage } from '@/types';

  let images = $state.raw<AdminImage[]>([]);
  let trashImages = $state.raw<AdminTrashImage[]>([]);
  let total = $state(0);
  let page = $state(1);
  let search = $state('');
  ...
  let restoreTarget = $state<AdminTrashImage | null>(null);
  let busy = $state(false);
  let debouncedSearch = $state('');
  let pageSize = $state(30);
  const pageSizeOptions = [10, 30, 50, 100];
  const searchDebounceMs = 300;
  const activeTab = $derived(routePage.url.searchParams.get('tab') === 'trash' ? 'trash' : 'active');
  const visibleImages = $derived(activeTab === 'trash' ? trashImages : images);
  ...
  async function load(currentPage = page, currentSearch = debouncedSearch, currentPageSize = pageSize, signal?: AbortSignal) {
    ...
    try {
      if (activeTab === 'trash') {
        const data = await adminGetTrashImages(preferences.adminToken, currentPage, currentPageSize, currentSearch || undefined, signal);
        if (signal?.aborted) return;
        trashImages = data.items;
        total = data.total;
        selected = new Set();
        return;
      }
  ...
  async function restoreOne(image: AdminTrashImage) {
    const token = preferences.adminToken;
    if (!token) return;
    await runAsyncAction({
      language: preferences.language,
      setBusy: (value) => (busy = value),
      successMessage: t(preferences.language, 'admin.imagesRestored', { uid: image.uid }),
      fallbackErrorKey: 'admin.imagesRestoreError',
      action: () => adminRestoreImage(token, image.uid),
      onSuccess: async () => {
        restoreTarget = null;
        await load();
      },
  ...
  $effect(() => {
    if (activeTab === 'active' || activeTab === 'trash') page = 1;
  });
...
<div class="space-y-6">
  ...
  <div class="flex flex-wrap gap-2 border-b-[3px] ink-line pb-4">
    <a class="studio-button" data-tone={activeTab === 'active' ? 'primary' : undefined} href="/admin/dashboard/images?tab=active"><Archive class="size-4" />{t(preferences.language, 'admin.imagesActive')}</a>
    <a class="studio-button" data-tone={activeTab === 'trash' ? 'primary' : undefined} href="/admin/dashboard/images?tab=trash"><Trash2 class="size-4" />{t(preferences.language, 'admin.imagesTrash')}</a>
  </div>
    ...
    <table class="w-full min-w-[760px] table-fixed border-collapse text-sm">
      <thead>
        <tr class="border-b-[3px] ink-line text-left text-xs font-black uppercase tracking-[0.08em] text-[hsl(var(--ink-muted))]">
          ...
          {#if activeTab === 'trash'}
            <th class="w-[16%] px-2 py-2" scope="col">{t(preferences.language, 'admin.imagesDeletedAt')}</th>
            <th class="w-[16%] px-2 py-2" scope="col">{t(preferences.language, 'admin.imagesPurgeAfter')}</th>
          {/if}
          <th class="w-52 px-2 py-2 text-right" scope="col">{t(preferences.language, 'admin.imagesTableActions')}</th>
        </tr>
      </thead>
      <tbody>
        {#each visibleImages as image (image.uid)}
          <tr class="studio-table-row align-middle">
            <td class="py-2 pr-2">
              {#if activeTab === 'active'}
                <input type="checkbox" checked={selected.has(image.uid)} onchange={() => toggle(image.uid)} aria-label={t(preferences.language, 'admin.imagesSelect', { uid: image.uid })} />
              {/if}
            </td>
            <td class="px-1 py-2">
            ...
            {#if activeTab === 'trash'}
              <td class="px-2 py-2 text-xs font-bold">{image.deleted_at ? formatDate(image.deleted_at, preferences.language) : '—'}<span class="block truncate text-[hsl(var(--ink-muted))]" title={image.delete_reason || ''}>{image.delete_reason || '—'}</span></td>
              <td class="px-2 py-2 text-xs font-bold">{image.purge_after ? formatDate(image.purge_after, preferences.language) : '—'}<span class="block truncate text-[hsl(var(--ink-muted))]">{image.deleted_by || '—'}</span></td>
            {/if}
            <td class="px-2 py-2">
              {#if activeTab === 'trash'}
                <div class="flex justify-end gap-1.5"><button class="studio-button px-2 py-1.5 text-xs" data-tone="green" type="button" onclick={() => (restoreTarget = image)}><RotateCcw class="size-4" />{t(preferences.language, 'admin.imagesRestore')}</button></div>
              {:else}
  ...
  <ConfirmDialog
    ...
    title={t(preferences.language, 'admin.imagesRestoreConfirm')}
    description={restoreTarget?.uid ?? ''}
    confirmLabel={t(preferences.language, 'admin.imagesRestore')}
    cancelLabel={t(preferences.language, 'common.cancel')}
    tone="primary"
    {busy}
    onClose={() => (restoreTarget = null)}
    onConfirm={() => restoreTarget && restoreOne(restoreTarget)}
  />
  <ConfirmDialog
