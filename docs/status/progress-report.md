# 整改与扩展任务进度报告

> 报告日期：2026-05-22
>
> 报告版本：v1.0（初始状态）

## 总体进度

| 优先级 | 总任务数 | 未开始 | 进行中 | 已完成 | 完成率 |
|--------|----------|--------|--------|--------|--------|
| P0 | 9 | 0 | 0 | 8（另 1 项已取消） | 89%（取消项不计入剩余） |
| P1 | 9 | 9 | 0 | 0 | 0% |
| P2 | 8 | 8 | 0 | 0 | 0% |
| **合计** | **26** | **17** | **0** | **8（另 1 项已取消）** | **31%（取消项不计入剩余）** |

## 各任务状态

### P0：必备整改项

| 任务 | 状态 | 开发 | 测试 | 复查 | 备注 |
|------|------|------|------|------|------|
| 上传资源保护 | 已完成 | 完成 | 通过 | 通过 | 新增像素上限、AVIF 并发与超时保护；`go test ./...`、`npm run typecheck` 通过 |
| 文件真实性校验 | 已完成 | 完成 | 通过 | 通过 | 服务层新增真实图片/MIME 交叉校验；`go test ./...`、`go build ./...`、`gofmt -l ./cmd ./internal` 通过 |
| URL 上传前端下载改造 | 已完成 | 完成 | 通过 | 通过 | 用户要求不再由后端下载 URL；已移除 `POST /v1/image/url` 与后端远端抓取逻辑，前端改为浏览器下载图片后复用 multipart 上传；后端 242 项测试、前端 lint/typecheck/55 项测试/build 均通过 |
| Token 治理基础 | 已完成 | 完成 | 通过 | 通过 | 新增 token_usage/token_controls、SHA-256 token hash、上传前禁用检查、上传后使用统计及管理员禁用/恢复 API；`go test ./...`、`gofmt -l ./cmd ./internal` 通过 |
| 默认密码安全改造 | 已完成 | 完成 | 通过 | 通过 | 持久化默认密码状态；默认密码下禁止 runtime/storage 高危配置变更但允许改密；`go test ./...`、`gofmt -l ./cmd ./internal`、`npm run typecheck`、`npm run build:backend` 通过 |
| 软删除与回收站 | 已取消/已移除 | 完成 | 通过 | 通过 | 用户确认不需要回收站；已移除软删除字段、回收站/恢复 API、前端入口与相关测试，删除改回直接删除数据库记录；后端 250 项测试、前端 lint/typecheck/55 项测试/build 均通过 |
| SQLite 核心索引 | 已完成 | 完成 | 通过 | 通过 | 已补齐核心索引；回收站取消后不再创建 `deleted_at` 复合索引 |
| 配置审计日志 MVP | 已完成 | 完成 | 通过 | 通过 | 新增 config_audit_logs、runtime/storage 配置变更 before/after 审计、管理员查询 API 与敏感字段遮罩；`go test ./...`、`gofmt -l ./cmd ./internal` 通过 |
| 存储健康检查 MVP | 已完成 | 完成 | 通过 | 通过 | 新增 storage_health_checks、手动检测/列表 API、后台 5 分钟心跳、小对象写读删探测与连续失败计数；`go test ./...`、`gofmt -l ./cmd ./internal` 通过 |

### P1：高价值扩展

| 任务 | 状态 | 开发 | 测试 | 复查 | 备注 |
|------|------|------|------|------|------|
| AVIF 异步转换队列 | 未开始 | — | — | — | |
| 原图备份 | 未开始 | — | — | — | |
| 多格式输出 WebP | 未开始 | — | — | — | |
| 图片审核 | 未开始 | — | — | — | |
| 批量打包下载 | 未开始 | — | — | — | |
| 存储回退策略 | 未开始 | — | — | — | |
| Prometheus 指标 | 未开始 | — | — | — | |
| 请求追踪与结构化日志 | 未开始 | — | — | — | |
| 滥用统计聚合 | 未开始 | — | — | — | |

### P2：长期能力

| 任务 | 状态 | 开发 | 测试 | 复查 | 备注 |
|------|------|------|------|------|------|
| JPEG-XL 支持 | 未开始 | — | — | — | |
| 短链服务 | 未开始 | — | — | — | |
| 相册/集合管理 | 未开始 | — | — | — | |
| 图片访问统计 | 未开始 | — | — | — | |
| 图片处理工具 | 未开始 | — | — | — | |
| 存储迁移与复制 | 未开始 | — | — | — | |
| API Key / 用户空间 | 未开始 | — | — | — | |
| CDN 预热任务 | 未开始 | — | — | — | |

## 风险与阻塞项

当前无阻塞项。

## 下一阶段动作

**按优先级排序开始开发。**

### 推荐启动顺序（第一期：安全与稳定性基线，约 2-3 周）

1. **SQLite 核心索引** — 已完成（基础设施先行，无依赖）
2. **上传资源保护** — 已完成
3. **文件真实性校验** — 已完成
4. **URL 上传前端下载改造** — 已完成
5. **默认密码安全改造** — 已完成
6. **Token 治理基础** — 已完成
7. **配置审计日志 MVP** — 已完成
8. **软删除与回收站** — 已取消/已移除（用户确认不需要）
9. **存储健康检查 MVP** — 已完成

### 后续迭代

- **第二期（3-4 周）**：AVIF 异步队列 → 原图备份 → 存储回退策略 → 存储健康检查指标/告警完善
- **第三期（4-6 周）**：WebP 多格式 → 图片审核 → 批量打包下载 → Prometheus 指标 → 滥用统计
- **第四期（长期）**：短链、相册、访问统计、图片处理、存储迁移、API Key、CDN 预热

---

> 此报告将在每个子任务完成后更新;每完成一个子任务，将提交一次代码;
