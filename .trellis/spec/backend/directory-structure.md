...
This layout matches the responsibilities implemented from [README.md](../../../README.md):
...
- client IP resolution (trusted proxy support exists, but current startup passes no trusted proxy CIDRs)
...
- `backend/internal/http/clientip/` owns client IP resolution. Startup wires `NewResolver(nil, func() string { return settingsManager.Current().RealIPSource })`, so the default runtime setting uses `RemoteAddr`, while admin-selected forwarding headers hot-reload through SQLite settings. Handlers and middleware must use the resolver instead of trusting raw `X-Forwarded-For` values.
...
- `backend/internal/service/` contains business workflows such as upload deduplication, temporary upload-source preparation (including temp-file spooling for reader-backed uploads), delete reference counting, config updates, admin status aggregation, IP bans, abuse statistics, Cloudflare image cache purge, and storage health checks.
- `backend/internal/repository/` owns SQLite access. Keep SQL and transaction boundaries here or in closely related repository helpers, including the runtime-managed `storage_configs` catalog, `storage_health_checks` history, plus image/storage-key backfill logic.
- `backend/internal/cache/` owns Redis key conventions and narrow cache seams (`ImageLookupCache`, `ImagePreheatCache`, `MD5MappingCache`, `MD5MappingPreheatCache`, `HealthCache`). MD5 mappings accept `model.MD5MappingKey`; callers must not hand-write cache key strings.
...
- `backend/internal/service/admin_service.go` implements admin status, image management, storage configuration, IP-ban operations, abuse statistics, Cloudflare image cache purge, storage health delegation, and runtime settings.
...
- `backend/internal/repository/repository.go` plus focused repository files isolate reads/writes to the `images`, `config`, `storage_configs`, `announcements`, `ip_bans`, and `storage_health_checks` tables.
...
- API paths keep API 404 behavior instead of receiving `index.html`:
  ...
  - `/v1/*`
  - `/i/*`
  - `/admin/config`, `/admin/config/*`, `/admin/system-settings`, `/admin/storage/*`, `/admin/cloudflare/*`
