...
- Treat the schema and flow in [README.md](../../../README.md) plus the checked-in backend code as authoritative.
...
- Runtime storage catalog edits are coordinated by the service-layer storage catalog module: validate the complete post-change catalog first, then write SQLite, then reload `StorageManager`. Legacy flat admin config input is only an adapter into that catalog intent and must not duplicate catalog rules.
...
- `backend/internal/repository/image_repository.go` — `InsertImage`, `FindByUID`, `FindByMD5AndStorageKey`, `DeleteByUID`, `CountByMD5`, `CountByMD5AndStorageKey`, `CountByStoredFile`, `SearchImages`, `ListAllImages`, `ImageSummaryByIP`, `ListImagesByIP`, and admin-oriented image listing methods.
- `backend/internal/repository/config_repository.go` — `GetAllConfig`, `GetConfigValue`, `SetConfigValue`, `UpsertConfigValues`, `InsertMissingConfigValues`.
- `backend/internal/repository/storage_repository.go` — `InitializeStorageCatalog`, `ListStorageConfigs`, `GetStorageConfigByKey`, `CreateStorageConfig`, `UpdateStorageConfig`, `UpdateStorageConfigAndSetDefault`, `DeleteStorageConfig`, `SetDefaultStorageConfig`, `CountImagesByStorageKey`.
- `backend/internal/service/storage_catalog.go` — service-layer storage catalog rules for create/patch/delete/default switch, secret-mask preservation, in-use backend immutability, whole-catalog reload validation, SQLite write, and `StorageManager` reload sequencing.
- `backend/internal/repository/ip_ban_repository.go` — `CreateIPBan`, `ListIPBans`, `GetIPBan`, `FindActiveIPBanByHash`, `DeleteIPBan`, `ActiveIPBansByHash`, `CountActiveIPBans`.
- `backend/internal/repository/announcement_repository.go` — `ListPublicAnnouncements`, `ListAnnouncements`, `GetAnnouncement`, `CreateAnnouncement`, `UpdateAnnouncement`, `DeleteAnnouncement`, `ArchiveAnnouncement`.
- `backend/internal/repository/storage_health_repository.go` — `UpsertStorageHealthCheck`, `GetStorageHealthCheck`, `GetStorageHealthCheckByID`, `ListStorageHealthChecks`, `ListStorageHealthHistory`.
...
- `backend/internal/repository/migration.go` must keep `CurrentSchemaVersion` aligned with the physical schema. `r.Migrate(ctx)` must set `PRAGMA user_version = CurrentSchemaVersion`, and repository tests must assert `SchemaVersion(ctx)` matches after migration.
...
- tables: `images`, `config`, `storage_configs`, `announcements`, `ip_bans`, `storage_health_checks`
- image columns: `uid`, `token`, `storage_key`, `storage_backend`, `file_path`, `mime_type`, `size`, `md5_hash`, `ip_address`, `created_at`
- IP-ban columns: `ip_hash`, `ip_address`, `reason`, `expires_at`, `created_at`, `updated_at`
- Storage-health columns: `storage_key`, `status`, `latency_ms`, `error_message`, `consecutive_failures`, `created_at`, `updated_at`
...
- Storage secret fields returned as masked values from admin views must preserve the existing stored secret when submitted back unchanged; callers should not need to know this rule.
- Storage catalog mutations must validate the complete post-change catalog before any SQLite write so invalid provider settings or impossible default switches do not partially persist patches. After a successful SQLite write, `StorageManager` must reload from SQLite; reload failure is surfaced as `dependency_unavailable`, with SQLite left as source of truth for the next startup/reload.
...
- Store IP bans with `ip_hash` for lookup and `ip_address` as the single persisted/displayed IP field.
...
- `images.uid` stores the opaque public UID token, not the deobfuscated
...
- `uid:{uid}` Redis keys use the same opaque public UID token stored in SQLite.
- Scoped `md5:{storage_key}:{hash}` Redis keys must point to the XOR-obfuscated public UID token that owns the deduplicated object for that storage instance.
...
- Deduplication is scoped by the resolved `storage_key`. Same original bytes plus same storage key **reuse the existing record directly** — the duplicate upload returns the same UID and URL as the first upload, without creating a new database row or a new physical object. Same original bytes plus a different storage key must store a separate physical object in the selected storage and return a new UID.
- `duplicate=true` in the upload response indicates that the backend returned an existing record without writing new data. The UID and URL are identical to the original upload.
...
- Public storage options are returned only through `GET /v1/runtime-settings` as `storage.options`. They expose safe display fields: `storage_key`, `name`, `storage_backend`, and `is_default`; do not reintroduce `/v1/storage-options` without adding route, tests, and docs.
- The first persisted object for a unique upload must use that row's generated XOR-obfuscated public UID as the filename/key base; object keys and stored paths must end in `.avif`.
...
- Environment keys:
  - `UID_PREFIX`: required contract value prefix for deobfuscated plaintext, default `omeo_`
    - Trailing underscores are normalized so the deobfuscated plaintext keeps one separator before the 8-character short id.
  - `UID_ENCRYPTION_KEY`: required XOR obfuscation secret material for the raw UID plaintext (legacy name, not strong encryption); startup must fail when it is empty or shorter than the configured minimum, and it must not fall back to `JWT_SECRET`.
...
- Invalid XOR-obfuscated public UID, prefix mismatch, or missing `.avif` route suffix on public image routes -> `not_found`
- Redis miss with SQLite hit -> continue and repopulate cache

### 5. Good / Base / Bad Cases

- Good: upload PNG -> store `.avif` under the first generated XOR-obfuscated public UID filename/key, return opaque `uid`, public URL ends in `.avif`
- Base: duplicate upload to the same resolved storage key -> **return the existing record directly** (same UID, same URL, `duplicate=true`) without creating a new row or new physical object
...
- Public runtime-settings test asserts `storage.options` omits secret and path fields while marking the current default storage
- Resolve/delete tests assert bare `/i/{uid}` is rejected while `/i/{uid}.avif` succeeds
- Preheat test asserts Redis repopulates `uid:{uid}` and scoped `md5:{storage_key}:{hash}` using XOR-obfuscated public UIDs
...
- Hash the original upload bytes first, short-circuit duplicates before transformation, then persist the first AVIF object under its generated XOR-obfuscated public UID key and reuse that same stored object for later duplicate rows while keeping the XOR-obfuscated public UID token contract consistent in SQLite, Redis, API responses, and public routes
...
## Scenario: Storage Health Checks

### 1. Scope / Trigger

- Trigger: code changes storage provider validation, admin storage health APIs, heartbeat behavior, or the `storage_health_checks` SQLite table.

### 2. Signatures

- SQLite table: `storage_health_checks(id, storage_key, status, latency_ms, error_message, consecutive_failures, created_at, updated_at)`.
- Repository: `UpsertStorageHealthCheck`, `GetStorageHealthCheck`, `GetStorageHealthCheckByID`, `ListStorageHealthChecks`, `ListStorageHealthHistory`.
- Admin APIs: `GET /admin/storage/health`, `GET /admin/storage/:key/health-history`, `POST /admin/storage/:key/health-check`, `POST /admin/storage/health-check-all`.
- Frontend type: `AdminStorageHealthCheck` in `frontend/src/lib/types/index.ts`.

### 3. Contracts

- Store every probe as an append-only history row; latest status is selected by newest row per `storage_key`.
- `storage_key` must reference a runtime storage instance key, not backend type.
- `status` is a numeric backend enum; frontend translates it for display.
- `latency_ms` is the measured probe latency; failed probes keep an `error_message` suitable for admin display.
- `consecutive_failures` increments from the previous latest row for the same storage key and resets to `0` when healthy.
- Heartbeat checks must not mutate storage catalog config; they only append health rows.

### 4. Validation & Error Matrix

- Unknown `storage_key` in manual check -> `not_found`; unknown history query returns an empty list unless repository fails. Do not create a fake storage config.
- Provider probe failure -> persist an unhealthy row with error message and incremented failures; API request itself may still return a successful health payload.
- SQLite insert/select failure -> `dependency_unavailable`.
- Missing prior health row -> first unhealthy row starts with `consecutive_failures = 1`; first healthy row has `0`.

### 5. Good/Base/Bad Cases

- Good: admin clicks “check all”, each configured storage key gets one new history row and latest state updates.
- Base: no previous checks exist; `GET /admin/storage/health` returns one placeholder per configured storage key with unavailable/not-checked status until heartbeat/manual checks run.
- Bad: overwriting one row in place, losing latency/error history needed by the UI chart.

### 6. Tests Required

- Repository test verifies upsert appends rows, latest selection returns newest per key, and failure count increments/resets.
- Service/API test verifies manual check returns `AdminStorageHealthCheck` fields and unknown keys are rejected.
- Frontend API/typecheck verifies `AdminStorageHealthCheck[]` is used by storage health helper functions.
...
```sql
UPDATE storage_health_checks SET status = ?, updated_at = ? WHERE storage_key = ?;
```

#### Correct

```sql
INSERT INTO storage_health_checks(storage_key, status, latency_ms, error_message, consecutive_failures, created_at, updated_at)
VALUES (?, ?, ?, ?, ?, ?, ?);
```
...
- Allowing schema names in code to drift from the names documented in [README.md](../../../README.md).
