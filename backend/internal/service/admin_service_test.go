...
func TestAdminTrashImagesAndRestoreImage(t *testing.T) {
	ctx := context.Background()
	dir := t.TempDir()
	...
	imageService := NewImageServiceWithCaches(repo, cacheStore.images, cacheStore.images, cacheStore.md5, cacheStore.md5, manager, settingsManager, nil, func(string) error { return nil }, slog.New(slog.NewTextHandler(ioDiscard{}, nil)))
	adminService := NewAdminService(repo, manager, settingsManager, imageService, "secret", AdminEnvMetadata{})
	...
	record := model.ImageRecord{
		...
		FilePath:       "2026/05/uid-trash-admin.avif",
		MIMEType:       "image/avif",
		Size:           1,
		MD5Hash:        "hash-trash-admin",
	}
	if err := repo.InsertImage(ctx, record); err != nil {
		t.Fatalf("InsertImage returned error: %v", err)
	}
	if err := imageService.deleteRecord(ctx, record); err != nil {
		t.Fatalf("deleteRecord returned error: %v", err)
	}

	defaultList, err := adminService.Images(ctx, 1, 20, "uid-trash-admin")
	if err != nil {
		t.Fatalf("Images returned error: %v", err)
	}
	if defaultList.Total != 0 || len(defaultList.Items) != 0 {
		t.Fatalf("expected default admin list to exclude trash, got %+v", defaultList)
	}
	trash, err := adminService.TrashImages(ctx, 1, 20, "uid-trash-admin")
	if err != nil {
		t.Fatalf("TrashImages returned error: %v", err)
	}
	if trash.Total != 1 || len(trash.Items) != 1 || trash.Items[0].DeletedAt == nil {
		t.Fatalf("expected deleted image in trash, got %+v", trash)
	}
	if err := adminService.RestoreImage(ctx, "uid-trash-admin"); err != nil {
		t.Fatalf("RestoreImage returned error: %v", err)
	}
	if restored, err := repo.FindByUID(ctx, "uid-trash-admin"); err != nil || restored.DeletedAt != nil {
		t.Fatalf("expected restored active image, record=%+v err=%v", restored, err)
	}
	if !cacheStore.hasCachedImage("uid-trash-admin") {
