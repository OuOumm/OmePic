...
func TestMigrateCreatesTokenGovernanceTablesIdempotently(t *testing.T) {
	ctx := context.Background()
	repo, err := New(filepath.Join(t.TempDir(), "test.sqlite"))
	if err != nil {
		t.Fatalf("New returned error: %v", err)
	...
	for table, columns := range map[string][]string{
		"token_usage":    {"token_hash", "token_preview", "upload_count", "total_bytes", "last_ip", "last_used_at", "created_at", "updated_at"},
		"token_controls": {"token_hash", "token_preview", "disabled", "reason", "disabled_at", "created_at", "updated_at"},
	} {
		for _, column := range columns {
			exists, err := testTableColumnExists(ctx, repo.db, table, column)
			if err != nil {
				t.Fatalf("testTableColumnExists(%s.%s) returned error: %v", table, column, err)
			}
			if !exists {
...
func TestTokenUsageRepositoryAggregatesAndControls(t *testing.T) {
	ctx := context.Background()
	repo, err := New(filepath.Join(t.TempDir(), "test.sqlite"))
	if err != nil {
		t.Fatalf("New returned error: %v", err)
	...
	first := model.TokenUsage{TokenHash: "hash", TokenPreview: "****oken", TotalBytes: 10, LastIP: "127.0.0.1"}
	second := model.TokenUsage{TokenHash: "hash", TokenPreview: "****oken", TotalBytes: 15, LastIP: "127.0.0.2"}
	if err := repo.UpsertTokenUsage(ctx, first); err != nil {
		t.Fatalf("first UpsertTokenUsage returned error: %v", err)
	}
	if err := repo.UpsertTokenUsage(ctx, second); err != nil {
		t.Fatalf("second UpsertTokenUsage returned error: %v", err)
	}
	if err := repo.SetTokenDisabled(ctx, "hash", "****oken", "abuse", true); err != nil {
		t.Fatalf("SetTokenDisabled returned error: %v", err)
	}

	entries, err := repo.ListTokenGovernance(ctx)
	if err != nil {
		t.Fatalf("ListTokenGovernance returned error: %v", err)
	}
	if len(entries) != 1 {
		t.Fatalf("expected 1 entry, got %d", len(entries))
	}
	entry := entries[0]
	if entry.UploadCount != 2 || entry.TotalBytes != 25 || entry.LastIP != "127.0.0.2" || !entry.Disabled || entry.Reason != "abuse" {
		t.Fatalf("unexpected token governance entry: %+v", entry)
	}
}
