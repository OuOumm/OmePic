...
func TestStorageHealthHistoryKeepsRecordsAndLatestList(t *testing.T) {
	...
	now := time.Now().UTC()
	oldCheck, err := repo.UpsertStorageHealthCheck(ctx, model.StorageHealthCheck{StorageKey: "local-default", Status: model.StorageHealthHealthy, LastCheckAt: now.Add(-25 * time.Hour), LatencyMS: 10})
	if err != nil {
		t.Fatalf("old upsert returned error: %v", err)
	}
	recentCheck, err := repo.UpsertStorageHealthCheck(ctx, model.StorageHealthCheck{StorageKey: "local-default", Status: model.StorageHealthUnavailable, LastCheckAt: now.Add(-1 * time.Hour), LatencyMS: 20, ErrorMessage: "boom"})
	if err != nil {
		t.Fatalf("recent upsert returned error: %v", err)
	}
	otherCheck, err := repo.UpsertStorageHealthCheck(ctx, model.StorageHealthCheck{StorageKey: "archive", Status: model.StorageHealthHealthy, LastCheckAt: now.Add(-30 * time.Minute), LatencyMS: 5})
	if err != nil {
